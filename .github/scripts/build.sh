...
cd $GITHUB_WORKSPACE/SW/compiler
make clean all
cd $GITHUB_WORKSPACE/SW/fs
python3 bin2c.py
cd ..
make clean all -f makefile.kernels RISCV_PATH=$GITHUB_WORKSPACE/riscv RISCV_NAME=riscv32-unknown-elf
make clean all