...
_share float8 histogram::histogram_lo[NUM_THREAD_PER_CORE*kHistogramBinSize];
_share float8 histogram::histogram_hi[NUM_THREAD_PER_CORE*kHistogramBinSize];
float8 histogram::histogram[kHistogramBinSize];
_share float histogram::in[kHistogramInSize*NUM_THREAD_PER_CORE];
float *histogram::p;
float *histogram::histogram_p;

// Module initialization

_kernel_ void histogram::init() {
   int i,ii;
   float8 *t;
   float8 *p_lo,*p_hi;
...
_kernel_ void histogram::accumulate() {
   int i,ii;
   float8 *p_lo;
   float8 *p_hi;
...
_kernel_ void histogram::done(_global int count) {
   int ii,i,j,idx;
   float8 *p_lo,*p_hi;
   ii=tid;
   p_lo=&histogram_lo[ii];
...
_kernel_ void histogram::adjust_extra_zero() {
