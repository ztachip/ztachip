...
void ztamTaskSpawn(void(*func)(void *,int), void *_p, uint32_t p2) {
   ...
   _taskSpawn(taskEntry, func, (uint32_t)_p, p2);
   _taskYield();
}
...
int ztamMsgqReadAvail(int queue) {
   return (int)(ZTAM_GREG(queue,REG_MSGQ_READ_AVAIL,0));
}

// Retrieve an integer from message queue with host CPU

int32_t ztamMsgqReadInt(int queue) {
   while((ZTAM_GREG(queue,REG_MSGQ_READ_AVAIL,0))==0) {
      ztamYield();
   }
   return (int32_t)(ZTAM_GREG(queue,REG_MSGQ_READ,0));
}


// Retrieve a pointer from message queue with host CPU

uint32_t ztamMsgqReadPointer(int queue) {
   while((ZTAM_GREG(queue,REG_MSGQ_READ_AVAIL,0))==0) {
      ztamYield();
   }
   return (uint32_t)(ZTAM_GREG(queue,REG_MSGQ_READ,0));
}
