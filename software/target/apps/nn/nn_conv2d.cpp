...
NeuralNetLayerConv2D::NeuralNetLayerConv2D(NeuralNet *nn,NeuralNetOperatorDef* def,ConvolutionType _type) : NeuralNetLayer(nn,def) {
   m_type=_type;
   m_fcnFunc=ztahostGetExportFunction("do_innerProduct");
   m_convFunc=ztahostGetExportFunction("do_convolution");
   m_convDepthwiseFunc=ztahostGetExportFunction("do_convolution_depthwise");
}
...
ZtaStatus NeuralNetLayerConv2D::Evaluate(int queue) {
   ...
   if(op->op==NeuralNetOperatorConv2D) {
      cmd=((*op->input_shape[0])[1]==1)?m_fcnFunc:m_convFunc;
   } else if(op->op==NeuralNetOperatorConvDepthWise) {
      cmd=m_convDepthwiseFunc;
   } else {
      assert(0); //????
   }
   if(cmd==m_fcnFunc) {
      ...
      ztahostMsgqWritePointer(queue,m_nn->BufferGetFlat(op->input[0])); // bot
      ztahostMsgqWritePointer(queue,outInterleave?m_nn->BufferGetInterleave(op->output[0]):m_nn->BufferGetFlat(op->output[0])); // top
      ztahostMsgqWriteInt(queue,(*op->output_shape[0])[3]); // topcnt
      ztahostMsgqWriteInt(queue,(*op->input_shape[0])[3]); // botcnt
      ztahostMsgqWriteInt(queue,m_strategy.fcn.coef_dim[0]); // topcnt
      ztahostMsgqWriteInt(queue,m_strategy.fcn.coef_dim[1]); // botcnt
      ...
      ztahostMsgqWritePointer(queue,m_shmBiasHi); // bias
      ztahostMsgqWritePointer(queue,m_shmBiasLo); // bias
      ztahostMsgqWritePointer(queue,(in_fmt==kTensorFormatInterleaved)?m_nn->BufferGetInterleave(op->input[0]):m_nn->BufferGetFlat(op->input[0])); // bot
      ztahostMsgqWritePointer(queue,m_nn->BufferGetFlat(op->output[0])); // top
      ztahostMsgqWritePointer(queue,m_nn->BufferGetInterleave(op->output[0])); // top
      ztahostMsgqWriteInt(queue,(*op->u.conv.filter_shape)[1]); // kernel size
      ztahostMsgqWriteInt(queue,(*op->output_shape[0])[3]); // topcnt
      ztahostMsgqWriteInt(queue,(*op->output_shape[0])[1]); // topdim
      ztahostMsgqWriteInt(queue,(*op->input_shape[0])[3]); // botcnt
      ztahostMsgqWriteInt(queue,(*op->input_shape[0])[1]+2*op->u.conv.pad_w); // botdim
      ztahostMsgqWriteInt(queue,op->u.conv.input_offset); 
      ztahostMsgqWriteInt(queue,op->u.conv.output_scale); // activation offset
      ztahostMsgqWritePointer(queue,m_shmSpu);   
      ztahostMsgqWriteInt(queue,1); // group
      ztahostMsgqWriteInt(queue,op->u.conv.stride_w); // stride
      ztahostMsgqWriteInt(queue,op->u.conv.pad_w); // pad
      ztahostMsgqWriteInt(queue,m_strategy.conv.dx);
      ztahostMsgqWriteInt(queue,m_strategy.conv.dycnt);
