...
ZtaStatus TfliteNn::Verify() {
   ...
   if(m_model->version() != TFLITE_SCHEMA_VERSION) {
      printf("Unsupported schema version \r\n");
      return ZtaStatusFail;
   }
   ...
   for (uint32_t subgraph_index = 0; subgraph_index < subgraphs->Length();
       ++subgraph_index) {
      const tflite::SubGraph* subgraph = (*subgraphs)[subgraph_index];
      auto operators = subgraph->operators();
      auto tensors = subgraph->tensors();
      if (!operators || !tensors) {
         printf("Erro1r loading tflite file \r\n");
         return ZtaStatusFail;
      }
      ...
      for (uint32_t i = 0; i < tensors->Length(); ++i) {
         ...
         if(tensor->quantization()->zero_point() && tensor->quantization()->scale()) {
            for(uint32_t j=0;j < tensor->quantization()->zero_point()->Length();j++) {
         ...
         for(uint32_t j=0;j < tensor->shape()->Length();j++) {
      ...
      for (uint32_t i = 0; i < operators->Length(); ++i) {
         ...
         switch(def.op) {
            ...
            case NeuralNetOperatorConcatenation: {
               ...
               assert(op->inputs()->Length() < TFLITE_MAX_NUM_INPUT);
               def.u.concat.num_input=op->inputs()->Length();
               for(int i=0;i < (int)op->inputs()->Length();i++) {
...
//               def.u.detection.score_threshold=(float)1e-8;
            ...
            case NeuralNetOperatorReshape: {
               ...
               for(int i=0;i < (int)op->outputs()->Length();i++) {
            ...
            case NeuralNetOperatorAdd: {
               ...
               assert(add_params);
               assert(op->inputs()->Length()==2);
               assert(op->outputs()->Length()==1);
               def.input.push_back(op->inputs()->Get(0));
               def.input.push_back(op->inputs()->Get(1));
               def.output.push_back(op->outputs()->Get(0));
