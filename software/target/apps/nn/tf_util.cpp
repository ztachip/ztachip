...
NeuralNetOperator TfliteNn::ParseOpcode(const tflite::OperatorCode *opcode)
{
   switch(opcode->builtin_code()) {
      ...
      default:
         printf("Fail to load model. Unsupported operator %d\r\n",opcode->builtin_code());
         return NeuralNetOperatorUnknown; 
   }
}
...
ZtaStatus TfliteNn::PopulateConvolutionQuantizationParams(
    ...
    const NeuralNetActivation activation, int32_t* multiplier, int* shift,
  ...
  if(num_channels != 1) {
     printf("Dont support per channel quantization \r\n");
