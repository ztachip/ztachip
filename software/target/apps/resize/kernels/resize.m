...
static void resize(void *_p,int pid) {
   ...
   dy=(TILE_DIM+TILE_PAD)*VECTOR_WIDTH;
   dy1=TILE_DIM*VECTOR_WIDTH;
   dx2=NUM_PCORE*req->scale;
...
void do_resize_box(int queue) {
   ...
   req.filter[0]=ztamMsgqReadPointer(queue);
   req.filter[1]=ztamMsgqReadPointer(queue);
   req.filteri[0]=ztamMsgqReadPointer(queue);
   req.filteri[1]=ztamMsgqReadPointer(queue);
   req.filterLen[0]=ztamMsgqReadInt(queue);
   req.filterLen[1]=ztamMsgqReadInt(queue);
   req.spu=ztamMsgqReadPointer(queue);
   req.nchannel=ztamMsgqReadInt(queue);
   req.src_w=ztamMsgqReadInt(queue);
   req.src_h=ztamMsgqReadInt(queue);
