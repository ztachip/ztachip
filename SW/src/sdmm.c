...
#define MOSI(a)   (SPI=((a)?(SPI|0x40):(SPI&(~0x40))),APB[APB_LED]=SPI)
#define CS_SD(a)  (SPI=((a)?(SPI|0x80):(SPI&(~0x80))),APB[APB_LED]=SPI)
#define CS_ESP(a) (SPI=((a)?(SPI|0x100):(SPI&(~0x100))),APB[APB_LED]=SPI)
#define MISO()    ((APB[APB_PB] & 0x10)?1:0)
...
static void dly_us (UINT n)	/* Delay n microseconds (avr-gcc -Os) */
{
	...
	tick = Time2Get();
	for(;;) {
		elapsedTime = (((int)Time2Get()-(int)tick));
		if(elapsedTime >= n)
			break;
	}
}
...
)
{
	...
	do {
		...
		MOSI((d & 0x80)?1:0);SCK(1);SCK(0);
		MOSI((d & 0x40)?1:0);SCK(1);SCK(0);
		MOSI((d & 0x20)?1:0);SCK(1);SCK(0);
		MOSI((d & 0x10)?1:0);SCK(1);SCK(0);
		MOSI((d & 0x08)?1:0);SCK(1);SCK(0);
		MOSI((d & 0x04)?1:0);SCK(1);SCK(0);
		MOSI((d & 0x02)?1:0);SCK(1);SCK(0);
		MOSI((d & 0x01)?1:0);SCK(1);SCK(0);
	} while (--bc);
}
...
DSTATUS disk_initialize (
	...
	dly_us(10000);			/* 10ms */
	CS_SD(1);		/* Initialize port pin tied to CS */
	SCK(0);		/* Initialize port pin tied to SCLK */

	for (n = 10; n; n--) rcvr_mmc_slow(buf, 1);	/* Apply 80 dummy clocks and the card gets ready to receive command */
