...
void ztaInitPcore(uint16_t *_image) {
   ...
   c_len=((c_len+3)>>2)<<2; // Must be in multiple of 4xINT16 words

   // Set pcore process1's constant memory space
   // Set pcore process1's constant memory space

   // Tranfer must be done in 64bit transaction

   if(c_len > 0) {
      > DTYPE(INT16) PCORE[*].root.constant[0:c_len-1] <= DTYPE(INT16)MEM((uint32_t)c_p,c_len/4,4)[0:c_len/4-1][:];
   }
   ZTAM_GREG(0,REG_DP_VM_TOGGLE,0)=0;
   ...
   > PROG((pcoreLen>>1)) <= DTYPE(INT16)MEM((uint32_t)pcore_p,(pcoreLen>>1),4)[:][:];
   > BARRIER;
}
