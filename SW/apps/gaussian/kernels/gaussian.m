   ...
   for(ch=0;ch < req->nchannel;ch++) {
      ...
      for(y=from;y < to;y++) {
         for(x=0;x < dxcnt;x++) {
            ...
            >SHUFFLE DTYPE(UINT8) FOR(K=0:VECTOR_WIDTH-1) FOR(I=0:TILE_DY_DIM+2*pad-1) FOR(II=0:NUM_PCORE-1) FOR(J=pad:pad+TILE_DX_DIM-1) PCORE(NUM_PCORE)[II].iconv::inbuf(TILE_DY_DIM+2*pad,TILE_DX_DIM+2*pad,VECTOR_WIDTH)[I][J][K] <= 
