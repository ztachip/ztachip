      ...
      for(y=from;y < to;y++) {
         for(x=0;x < dxcnt;x++) {
            ...
            if(x>0) {
               ...
               >DTYPE(INT8)PCORE(NUM_PCORE)[0].iconv::inbuf(TILE_DY_DIM+2*pad,TILE_DX_DIM+2*pad,VECTOR_WIDTH)[:][0:pad-1][:] <= INT8(0);
            }

            // Copy input to PCORE array...
            >CONCURRENT DTYPE(UINT8) FOR(K=0:VECTOR_WIDTH-1) FOR(I=0:TILE_DY_DIM+2*pad-1) FOR(II=0:NUM_PCORE-1) FOR(J=pad:pad+TILE_DX_DIM-1) PCORE(NUM_PCORE)[II].iconv::inbuf(TILE_DY_DIM+2*pad,TILE_DX_DIM+2*pad,VECTOR_WIDTH)[I][J][K] <= 
            ...
            >CONCURRENT FOR(K=0:VECTOR_WIDTH-1) FOR(I=0:TILE_DY_DIM-1) FOR(II=0:NUM_PCORE-1) FOR(J=0:TILE_DX_DIM-1) DTYPE(UINT8) PCORE(NUM_PCORE)[II].iconv::outbuf(TILE_DY_DIM,TILE_DX_DIM,VECTOR_WIDTH)[I][J][K];
         }
      }
   }
}
