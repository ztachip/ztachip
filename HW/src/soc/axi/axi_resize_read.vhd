...
architecture rtl of axi_resize_read is
...
record
   rlast:axi_rlast_t;
   rid:axi_rid_t;
   rdata:std_logic_vector(MASTER_DATA_WIDTH-1 downto 0);
   rresp:axi_rresp_t;
end record;
...
signal axislave_cmd_pend_rec_read:axiread_cmd_rec_t;
signal axislave_cmd_pend_rec_write:axiread_cmd_rec_t;
signal axislave_cmd_pend_fifo_read:axiread_cmd_fifo_t;
signal axislave_cmd_pend_fifo_write:axiread_cmd_fifo_t;
signal axislave_cmd_pend_fifo_full:std_logic;
signal axislave_cmd_pend_fifo_empty:std_logic;
signal axislave_cmd_pend_fifo_wr:std_logic;
signal axislave_cmd_pend_fifo_rd:std_logic;
...
signal aximaster_rdata_r:STD_LOGIC_VECTOR(MASTER_DATA_WIDTH-1 downto 0);
signal aximaster_rresp_r:axi_rresp_t;
signal aximaster_rready:std_logic;

signal rvalid:std_logic;
signal rlast:std_logic;
signal rdata:std_logic_vector(SLAVE_DATA_WIDTH-1 downto 0);
...
axislave_rvalid_out <= rvalid;
axislave_rlast_out <= rlast;
axislave_rdata_out <= rdata;
axislave_arready_out <= arready;
...
aximaster_araddr_out <= axislave_cmd_rec_read.araddr(axislave_cmd_rec_read.araddr'length-1 downto MASTER_DATA_BYTE_WIDTH_L) & std_logic_vector(to_unsigned(0,MASTER_DATA_BYTE_WIDTH_L));
...
axislave_cmd_pend_fifo_wr <= (axislave_arvalid_in) and arready;
axislave_cmd_fifo_wr <= (axislave_arvalid_in) and arready;
...
--axislave_resp_fifo_rd <= rready and rvalid;
...
axislave_cmd_pend_rec_read <= unpack_cmd(axislave_cmd_pend_fifo_read);
axislave_cmd_pend_fifo_write <= pack_cmd(axislave_cmd_pend_rec_write);

-- FIFO to track pending requests

slave_cmd_pend_fifo:scfifo
   generic map
   (
   ...
   (
      ...
      reset_in=>reset_in,
      data_in=>axislave_cmd_pend_fifo_write,
      write_in=>axislave_cmd_pend_fifo_wr,
      read_in=>axislave_cmd_pend_fifo_rd,
      q_out=>axislave_cmd_pend_fifo_read,
      empty_out=>axislave_cmd_pend_fifo_empty,
      full_out=>axislave_cmd_pend_fifo_full,
      ravail_out=>open,
      wused_out=>open,
...
process(axislave_arlen_in,axislave_araddr_in,axislave_arid_in,axislave_arlock_in,
         axislave_arcache_in,axislave_arprot_in,axislave_arqos_in,axislave_arburst_in,
         axislave_arsize_in)
variable arlen_v:axi_arlen_t;
begin
   arlen_v := std_logic_vector(to_unsigned(0,SCALE_FACTOR_L)) & axislave_arlen_in(axislave_arlen_in'length-1 downto SCALE_FACTOR_L);
   if((unsigned('0' & axislave_arlen_in(SCALE_FACTOR_L-1 downto 0))+unsigned('0' & axislave_araddr_in(SLAVE_DATA_BYTE_WIDTH_L+SCALE_FACTOR_L-1 downto SLAVE_DATA_BYTE_WIDTH_L))) > to_unsigned(SCALE_FACTOR-1,SCALE_FACTOR_L+1)) then
      arlen_v := std_logic_vector(unsigned(arlen_v) + to_unsigned(1,arlen_v'length));
   end if;
   axislave_cmd_rec_write.araddr <= axislave_araddr_in(axislave_araddr_in'length-1 downto 0);
   axislave_cmd_rec_write.arlen <= arlen_v;
   ...
   axislave_cmd_rec_write.arsize <= "100"; -- axislave_arsize_in;

   axislave_cmd_pend_rec_write.araddr <= axislave_araddr_in;
   axislave_cmd_pend_rec_write.arlen <= axislave_arlen_in;
   axislave_cmd_pend_rec_write.arid <= axislave_arid_in;
   axislave_cmd_pend_rec_write.arlock <= axislave_arlock_in;
   axislave_cmd_pend_rec_write.arcache <= axislave_arcache_in;
   axislave_cmd_pend_rec_write.arprot <= axislave_arprot_in;
   axislave_cmd_pend_rec_write.arqos <= axislave_arqos_in;
   axislave_cmd_pend_rec_write.arburst <= axislave_arburst_in;
   axislave_cmd_pend_rec_write.arsize <= axislave_arsize_in;
end process;

arready <= (not axislave_cmd_fifo_full) and (not axislave_cmd_pend_fifo_full);

rready <= axislave_rready_in;

process(step_r,axislave_cmd_pend_rec_read,axislave_resp_fifo_empty,
      axislave_resp_rec_read,rready)
variable step_v:unsigned(SCALE_FACTOR_L-1 downto 0);
begin
   step_v := step_r(SCALE_FACTOR_L-1 downto 0) + unsigned(axislave_cmd_pend_rec_read.araddr(SLAVE_DATA_BYTE_WIDTH_L+SCALE_FACTOR_L-1 downto SLAVE_DATA_BYTE_WIDTH_L));
   axislave_resp_fifo_rd <= '0';
   axislave_cmd_pend_fifo_rd <= '0';
   rvalid <= '0';
   rdata <= (others=>'0');
   rlast <= '0';
   if(axislave_resp_fifo_empty='0') then
      rvalid <= '1';
      for I in 0 to SCALE_FACTOR-1 loop
         if(step_v=to_unsigned(I,step_v'length)) then
            rdata(SLAVE_DATA_WIDTH-1 downto 0) <= axislave_resp_rec_read.rdata((I+1)*SLAVE_DATA_WIDTH-1 downto I*SLAVE_DATA_WIDTH);
            exit;
         end if;
      end loop;
      if(step_v=to_unsigned(SCALE_FACTOR-1,step_v'length)) then
         axislave_resp_fifo_rd <= rready;
      end if;
      if(step_r=unsigned(axislave_cmd_pend_rec_read.arlen)) then
         rlast <= '1';
         axislave_resp_fifo_rd <= rready;
         axislave_cmd_pend_fifo_rd <= rready;
      else
         rlast <= '0';
      end if;
   end if;
