...
constant RAM_BYTE_WIDTH:integer:=16;
...
begin
   if(read_size_r="100") then
      rdata <= RAM(to_integer(read_addr_r)/16);
   elsif(read_size_r="011") then
      if(read_addr_r(3)='1') then
         rdata(63 downto 0) <= RAM(to_integer(read_addr_r)/16)(127 downto 64);
         rdata(127 downto 64) <= RAM(to_integer(read_addr_r)/16)(127 downto 64);
      else
         rdata(63 downto 0) <= RAM(to_integer(read_addr_r)/16)(63 downto 0);
         rdata(127 downto 64) <= RAM(to_integer(read_addr_r)/16)(63 downto 0);
      end if;
   else
      if(read_addr_r(3)='1') then
         if(read_addr_r(2)='1') then        
            rdata(31 downto 0) <= RAM(to_integer(read_addr_r)/16)(127 downto 96);
            rdata(63 downto 32) <= RAM(to_integer(read_addr_r)/16)(127 downto 96);
            rdata(95 downto 64) <= RAM(to_integer(read_addr_r)/16)(127 downto 96);
            rdata(127 downto 96) <= RAM(to_integer(read_addr_r)/16)(127 downto 96);
         else
            rdata(31 downto 0) <= RAM(to_integer(read_addr_r)/16)(95 downto 64);
            rdata(63 downto 32) <= RAM(to_integer(read_addr_r)/16)(95 downto 64);
            rdata(95 downto 64) <= RAM(to_integer(read_addr_r)/16)(95 downto 64);
            rdata(127 downto 96) <= RAM(to_integer(read_addr_r)/16)(95 downto 64);
         end if;
      else
         if(read_addr_r(2)='1') then  
            rdata(31 downto 0) <= RAM(to_integer(read_addr_r)/16)(63 downto 32);
            rdata(63 downto 32) <= RAM(to_integer(read_addr_r)/16)(63 downto 32);
            rdata(95 downto 64) <= RAM(to_integer(read_addr_r)/16)(63 downto 32);
            rdata(127 downto 96) <= RAM(to_integer(read_addr_r)/16)(63 downto 32);
         else
            rdata(31 downto 0) <= RAM(to_integer(read_addr_r)/16)(31 downto 0);
            rdata(63 downto 32) <= RAM(to_integer(read_addr_r)/16)(31 downto 0);
            rdata(95 downto 64) <= RAM(to_integer(read_addr_r)/16)(31 downto 0);
            rdata(127 downto 96) <= RAM(to_integer(read_addr_r)/16)(31 downto 0);
         end if;
      end if;
   end if;
end process;
      ...
      if(read_busy_r='1') then
         if(rready='1') then
            if(read_len_r=to_unsigned(0,read_len_r'length)) then
               ...
               if(read_size_r="100") then
                  read_addr_r <= read_addr_r+to_unsigned(16,read_addr_r'length);
      ...
      if(write_busy_r='1') then
         if(wready='1' and wvalid='1') then
            if(write_len_r=to_unsigned(0,write_len_r'length)) then
               ...
               if(write_size_r="100") then
                  write_addr_r <= write_addr_r+to_unsigned(16,write_addr_r'length);
                  write_len_r <= write_len_r-to_unsigned(1,write_len_r'length);
               elsif(write_size_r="011") then
            ...
            if(write_size_r="100") then
               for I in 0 to (RAM_BYTE_WIDTH-1) loop
                  if(wstrb(I)='1') then
                     RAM(to_integer(write_addr_r)/16)((8*(I+1)-1) downto 8*I) <= wdata((8*(I+1)-1) downto 8*I);
                  end if;
               end loop;
            elsif(write_size_r="011") then
               if(write_addr_r(3)='0') then
                  for I in 0 to 7 loop
                     if(wstrb(I)='1') then
                        RAM(to_integer(write_addr_r)/16)((8*(I+0+1)-1) downto 8*(I+0)) <= wdata((8*((I+0)+1)-1) downto 8*(I+0));
                     end if;
                  end loop;
               else
                  for I in 0 to 7 loop
                     if(wstrb(8+I)='1') then
                        RAM(to_integer(write_addr_r)/16)((8*(I+8+1)-1) downto 8*(I+8)) <= wdata((8*((I+8)+1)-1) downto 8*(I+8));
                     end if;
                  end loop;
               end if;
            else
               if(write_addr_r(3 downto 2)="00") then
                  for I in 0 to 3 loop
                     if(wstrb(I)='1') then
                        RAM(to_integer(write_addr_r)/16)((8*(I+0+1)-1) downto 8*(I+0)) <= wdata((8*((I+0)+1)-1) downto 8*(I+0));
                     end if;
                  end loop;
               elsif(write_addr_r(3 downto 2)="01") then
                  for I in 0 to 3 loop
                     if(wstrb(4+I)='1') then
                        RAM(to_integer(write_addr_r)/16)((8*(I+4+1)-1) downto 8*(I+4)) <= wdata((8*((I+4)+1)-1) downto 8*(I+4));
                     end if;
                  end loop;
               elsif(write_addr_r(3 downto 2)="10") then
                  for I in 0 to 3 loop
                     if(wstrb(8+I)='1') then
                        RAM(to_integer(write_addr_r)/16)((8*(I+8+1)-1) downto 8*(I+8)) <= wdata((8*((I+8)+1)-1) downto 8*(I+8));
                     end if;
                  ...
                  for I in 0 to 3 loop
                     if(wstrb(12+I)='1') then
                        RAM(to_integer(write_addr_r)/16)((8*(I+12+1)-1) downto 8*(I+12)) <= wdata((8*((I+12)+1)-1) downto 8*(I+12));
                     end if;
                  end loop;
               end if;
            end if;
